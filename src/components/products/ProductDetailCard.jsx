...
export const ProductDetailCard = ({productName="Astra", price=500, description="Dummy Description", initialStock=0, initialStatus=false, onUpdateProduct, onPatchProduct}) => {
    ...
    const { control, register, handleSubmit, formState: { errors }, reset } = useForm({
      ...
      const onSubmit = (data) => {
        ...
        if(fieldsInitialState.stock != stock && fieldsInitialState.status != status)
        {
            console.log('Do PUT : ', fieldsInitialState);
            onUpdateProduct({stock: stock, status: status ? 'ACTIVE' : 'INACTIVE'});
        }
        else if(fieldsInitialState.stock != stock || fieldsInitialState.status != status)
        {
          if(fieldsInitialState.stock != stock)onPatchProduct({stock: stock});
          if(fieldsInitialState.status != status)onPatchProduct({status: status ? 'ACTIVE' : 'INACTIVE'});
